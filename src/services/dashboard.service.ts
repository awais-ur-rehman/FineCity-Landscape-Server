...
export const getTasksByStatus = async (from: Date, to: Date, branchId?: string) => {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const match: any = { scheduledAt: { $gte: from, $lte: to } };
...
export const getUserPerformance = async (from: Date, to: Date, branchId?: string) => {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const match: any = {
