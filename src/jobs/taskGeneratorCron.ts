...
const startTaskGeneratorJob = () => {
  ...
  cron.schedule(schedule, async () => {
    ...
    try {
      const { created, schedules } = (await generateTasks()) as { created: number; schedules: number; newTasks: unknown[] };
...
const startDueNotifierJob = () => {
  ...
  cron.schedule(schedule, async () => {
    try {
      ...
      if (sent > 0) console.log(`[Cron:DueNotifier] Sent ${sent} due-now notifications`);
    } catch (err) {
...
const startAdvanceReminderJob = () => {
  ...
  cron.schedule(schedule, async () => {
    try {
      ...
      if (sent > 0) console.log(`[Cron:Reminder] Sent ${sent} advance reminders`);
    } catch (err) {
...
const startOverdueMarkerJob = () => {
  ...
  cron.schedule(schedule, async () => {
    try {
      ...
      if (missed > 0) console.log(`[Cron:Overdue] Marked ${missed} tasks as missed`);
    } catch (err) {
